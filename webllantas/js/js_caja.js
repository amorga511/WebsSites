...
appM.controller('appControl1', function($scope,$http) {
    ...
    $scope.f_save_cierre = function(){
        ...
        {
            $http.post('http://192.168.0.254/webllantas/server/svrTransacciones.php', {op:203, cierre:$scope.vCierre}).then(function(vResult){
                    alert(vResult.data);
                    msjR = vResult.data.split(',');
                        if(parseInt(msjR[0])==1){
                            reset_cierre();
    ...
    function fill_cierres(){
        $http.post('http://192.168.0.254/webllantas/server/svrConsultas.php', {op:107}).then(function(vResult){
            alert(vResult.data);
